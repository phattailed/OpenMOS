...
func TestUCS2BERoundTrip(t *testing.T) {
	original := []byte(`<mos><mosID>OPENMOS</mosID><ncsID>NCS_001</ncsID><messageID>1</messageID><roCreate><roID>RO1</roID><roSlug>Sample</roSlug></roCreate></mos>`)
...
// TestUCS2BENonASCIISurvives proves a roSlug containing accented/Unicode
...
func TestUCS2BENonASCIISurvives(t *testing.T) {
	...
	for _, text := range cases {
		original := []byte(`<mos><mosID>OPENMOS</mosID><ncsID>NCS</ncsID><messageID>m1</messageID><roCreate><roID>RO1</roID><roSlug>` + text + `</roSlug></roCreate></mos>`)
...
func TestUCS2BEFramerWaitsForOuterMOSClose(t *testing.T) {
	input := `<mos><mosID>MOS</mosID><ncsID>NCS</ncsID><roCreate><roID>RO1</roID><roSlug>Sample</roSlug>` +
		`<mosExternalMetadata><mosSchema>urn:example</mosSchema><mosPayload><mos><value>opaque</value></mos></mosPayload></mosExternalMetadata>` +
		`</roCreate></mos>`
	encoded, err := EncodeUCS2BE([]byte(input))
	if err != nil {
		t.Fatal(err)
	}
	framer := NewUCS2BEFramer()
	if err := framer.Append(encoded); err != nil {
		t.Fatal(err)
	}
	frame, complete, err := framer.Next()
	if err != nil {
		t.Fatal(err)
	}
	if !complete || string(bytes.TrimSpace(frame)) != input {
		t.Fatalf("framed inner close as an outer close: complete=%v frame=%q", complete, frame)
	}
}

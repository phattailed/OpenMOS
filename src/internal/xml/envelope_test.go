...
func TestParseEnvelope_RoCreate(t *testing.T) {
  ...
  <roCreate>
    <roID>RO_001</roID>
    <roSlug>Sample</roSlug>
    <roChannel>A</roChannel>
    <story>
      <storyID>STORY_001</storyID>
      <storySlug>Story One</storySlug>
    </story>
  </roCreate>
</mos>`
	...
	if roCreate.Slug != "Sample" {
		t.Errorf("roSlug = %q, want %q", roCreate.Slug, "Sample")
	}
...
func TestParseEnvelopePreservesNestedOperationPayload(t *testing.T) {
	input := `<mos><mosID>MOS</mosID><ncsID>NCS</ncsID><messageID>1</messageID>` +
		`<roCreate><roID>RO1</roID><roSlug>Sample</roSlug><mosExternalMetadata><mosSchema>urn:example</mosSchema>` +
		`<mosPayload><roCreate><field>opaque</field></roCreate></mosPayload></mosExternalMetadata></roCreate></mos>`
	_, message, _, err := ParseEnvelope([]byte(input))
	if err != nil {
		t.Fatal(err)
	}
	ro, ok := message.(RunningOrderInfo)
	if !ok {
		t.Fatalf("message type %T", message)
	}
	if got := ro.MosExternalMetadata[0].MosPayload.Raw; got != `<roCreate><field>opaque</field></roCreate>` {
		t.Fatalf("opaque payload = %q", got)
	}
}

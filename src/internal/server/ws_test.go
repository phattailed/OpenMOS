...
func testService() *service.MOSService {
	...
	return service.NewMOSService(roRepo, storyRepo, itemRepo, nil, eventBus)
}
...
func TestWSKeepAliveNoResponse(t *testing.T) {
	...
	keepAliveMsg := mosxml.WrapEnvelope("OPENMOS_TEST", "NCS_001", "", []byte("<keepAlive/>"))
...
func TestWSHeartbeatUsesBinaryEnvelope(t *testing.T) {
	_, baseURL, cancel := testServer(t)
	defer cancel()
	ctx, done := context.WithTimeout(context.Background(), 3*time.Second)
	defer done()
	conn, _, err := websocket.Dial(ctx, baseURL+"/mos?mosID=OPENMOS_TEST&ncsID=NCS_001&channel=ro", nil)
	if err != nil {
		t.Fatal(err)
	}
	defer conn.Close(websocket.StatusNormalClosure, "")
	request := mosxml.WrapEnvelope("OPENMOS_TEST", "NCS_001", "unusual-id", []byte(`<heartbeat/>`))
	...
	frameType, frame, err := conn.Read(ctx)
	if err != nil {
		t.Fatal(err)
	}
	if frameType != websocket.MessageBinary {
		t.Fatalf("response frame type: %v", frameType)
	}
	decoded, err := mosxml.DecodeUCS2BE(frame)
	if err != nil {
		t.Fatal(err)
	}
	var response struct {
	...
	if response.MessageID != "unusual-id" || response.Heartbeat.Time == "" || response.Heartbeat.Source != "" || response.Heartbeat.Timestamp != "" {
		t.Fatalf("unexpected heartbeat envelope: %+v", response)
	}
	// A distinct request sent immediately after the first still gets a reply.
	second := mosxml.WrapEnvelope("OPENMOS_TEST", "NCS_001", "next-id", []byte(`<heartbeat/>`))
	second, err = mosxml.EncodeUCS2BE(second)
	if err != nil {
		t.Fatal(err)
	}
	if err := conn.Write(ctx, websocket.MessageBinary, second); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(string(decoded), "<messageID>next-id</messageID>") {
		t.Fatalf("second heartbeat was not answered: %s", decoded)
	}
}
...
// the MOS 4 binary frame discipline.

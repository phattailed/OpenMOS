...
type WSSession struct {
	...
	closeOnce            sync.Once
	lastHeartbeatReply   mosxml.Heartbeat
	lastHeartbeatReplyID string
}

// NewWSServer creates a new WebSocket server.
func NewWSServer(cfg *config.Config, mosService *service.MOSService, dedup *MemoryDedupStore) *WSServer {
	return &WSServer{
		config:     cfg,
		service:    mosService,
		dedup:      dedup,
...
func (s *WSServer) handleSession(sess *WSSession) {
	...
	for {
		...
		case msg := <-msgCh:
			...
			// MOS 4.0 §2.1 mandates UCS-2 big-endian binary frames. Accept binary
...
func (s *WSServer) processMessage(ctx context.Context, sess *WSSession, data []byte) {
	...
	if env.MosID != sess.mosID || env.NcsID != sess.ncsID {
		sess.close()
		return
	}
	...
	case mosxml.Heartbeat:
		if m == sess.lastHeartbeatReply && env.MessageID == sess.lastHeartbeatReplyID {
			return
		}
		reply := mosxml.CreateHeartbeatResponse(m.RequestID)
		sess.lastHeartbeatReply = reply
		sess.lastHeartbeatReplyID = env.MessageID
		innerXML, err := xml.Marshal(reply)
		if err != nil {
			logger.Errorf("Failed to marshal heartbeat: %v", err)
			return
		}
		s.writeMessage(ctx, sess, mosxml.WrapEnvelope(sess.mosID, sess.ncsID, env.MessageID, innerXML))
		return
...
func (s *WSServer) handleRoCreate(ctx context.Context, sess *WSSession, env *mosxml.MosEnvelope, roInfo mosxml.RunningOrderInfo, innerOpXML []byte) {
	response := s.prepareROAck(ctx, sess, env, roInfo, innerOpXML)
	if response != nil {
		s.writeMessage(ctx, sess, response)
	}
}

func (s *WSServer) prepareROAck(ctx context.Context, sess *WSSession, env *mosxml.MosEnvelope, roInfo mosxml.RunningOrderInfo, innerOpXML []byte) []byte {
	s.roMu.Lock()
	defer s.roMu.Unlock()
	...
	case DedupConflict:
		...
		return s.errorROAck(sess, env.MessageID, roInfo.ID, "NACK: messageID conflict")
	}
	...
	if err != nil {
		...
		return s.errorROAck(sess, env.MessageID, roInfo.ID, roNackStatus(err))
	}

	// Send roAck only after successful persistence
	response, err := s.roAckEnvelope(sess, env.MessageID, roInfo.ID, "OK")
	if err != nil {
...
func (s *WSServer) roAckEnvelope(sess *WSSession, messageID, roID, status string) ([]byte, error) {
	ack := mosxml.CreateROAck(roID, status, nil)
	innerXML, err := xml.Marshal(ack)
	if err != nil {
		return nil, err
	}
	return mosxml.WrapEnvelope(s.config.MOS.ID, sess.ncsID, messageID, innerXML), nil
}

func (s *WSServer) errorROAck(sess *WSSession, messageID, roID, status string) []byte {
	response, err := s.roAckEnvelope(sess, messageID, roID, status)
	if err != nil {
		logger.Errorf("Failed to marshal roAck: %v", err)
		return nil
	}
...
// it as a binary WebSocket frame. MOS 4.0 §2.1 mandates UCS-2BE, so OpenMOS never emits

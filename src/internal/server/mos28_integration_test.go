...
func TestMOS28ROCreatePersistsAndAcknowledgesOnSameSocket(t *testing.T) {
  ...
  <roCreate>
	...
	// The envelope's mosID must be recorded on the running order. "The combination
	...
	if item := items.value("RO-41/STORY-1/ITEM-1"); item == nil || item.RawID != "ITEM-1" || item.StoryID != "RO-41/STORY-1" || item.ObjectID != "OBJ-1" || item.EditorialDuration != 25 || item.Duration != 0 {
...
func TestMOS28RejectsZeroMessageID(t *testing.T) {
	tcpServer, _, _, _ := startMOS28Server(t)
	conn := dialMOS28(t, tcpServer)
	const request = `<mos><mosID>openmos.example.test</mosID><ncsID>example.test</ncsID><messageID>0</messageID><roCreate><roID>RO-0</roID><roSlug>Sample</roSlug></roCreate></mos>`
	writeMOS28ForTest(t, conn, request)
	_ = conn.SetReadDeadline(time.Now().Add(time.Second))
	if _, err := conn.Read(make([]byte, 1)); err == nil {
		t.Fatal("zero messageID was accepted")
	} else if netErr, ok := err.(net.Error); ok && netErr.Timeout() {
		t.Fatalf("zero messageID left connection open: %v", err)
	}
}

func TestMOS28ShutdownWithActiveClient(t *testing.T) {
	tcpServer, _, _, _ := startMOS28Server(t)
	conn := dialMOS28(t, tcpServer)
	writeMOS28ForTest(t, conn, `<mos><mosID>openmos.example.test</mosID><ncsID>example.test</ncsID><messageID>4</messageID><roCreate><roID>RO-SHUTDOWN</roID><roSlug>Sample</roSlug></roCreate></mos>`)
	readMOS28XMLForTest(t, conn, &struct{}{})
	done := make(chan error, 1)
	go func() { done <- tcpServer.Shutdown(context.Background()) }()
	select {
	case err := <-done:
		if err != nil {
			t.Fatal(err)
	...
	case <-time.After(2 * time.Second):
		t.Fatal("shutdown blocked with an active client")
	}
}

func TestMOS28ROReqAllDiscovery(t *testing.T) {
	tcpServer, runningOrders, _, _ := startMOS28Server(t)
	_, err := runningOrders.Create(context.Background(), &model.RunningOrder{ID: "RO-1", Slug: "Sample", Duration: 30})
	if err != nil {
		t.Fatal(err)
	}
	conn := dialMOS28(t, tcpServer)
	writeMOS28ForTest(t, conn, `<mos><mosID>openmos.example.test</mosID><ncsID>example.test</ncsID><roReqAll/></mos>`)
	var reply struct {
		ROListAll struct {
			Orders []struct {
				ID       string `xml:"roID"`
	...
	if len(reply.ROListAll.Orders) != 1 || reply.ROListAll.Orders[0].ID != "RO-1" || reply.ROListAll.Orders[0].Duration != "00:00:30" {
		t.Fatalf("unexpected roListAll: %+v", reply.ROListAll)
	}
}

func TestMOS28PreservesOpaqueExternalMetadata(t *testing.T) {
	tcpServer, runningOrders, stories, items := startMOS28Server(t)
	conn := dialMOS28(t, tcpServer)
	request := `<mos><mosID>openmos.example.test</mosID><ncsID>example.test</ncsID><roCreate><roID>RO-META</roID><roSlug>Sample</roSlug>` +
		`<mosExternalMetadata><mosScope>PLAYLIST</mosScope><mosSchema>urn:example:ro</mosSchema><mosPayload><field key="a">R</field></mosPayload></mosExternalMetadata>` +
		`<story><storyID>STORY</storyID><mosExternalMetadata><mosScope>STORY</mosScope><mosSchema>urn:example:story</mosSchema><mosPayload><nested><value>S</value></nested></mosPayload></mosExternalMetadata>` +
		`<item><itemID>ITEM</itemID><objID>OBJ</objID><mosID>openmos.example.test</mosID><mosExternalMetadata><mosScope>OBJECT</mosScope><mosSchema>urn:example:item</mosSchema><mosPayload><value>I</value></mosPayload></mosExternalMetadata></item></story></roCreate></mos>`
	writeMOS28ForTest(t, conn, request)
	var ack struct {
		ROAck struct {
			Status string `xml:"roStatus"`
	...
	if ack.ROAck.Status != "OK" {
		t.Fatalf("roCreate status: %s", ack.ROAck.Status)
	}
	if got := runningOrders.value("RO-META").ExternalMetadata[0].Payload; got != `<field key="a">R</field>` {
		t.Fatalf("RO payload: %q", got)
	}
	if got := stories.value("RO-META/STORY").ExternalMetadata[0].Payload; got != `<nested><value>S</value></nested>` {
		t.Fatalf("story payload: %q", got)
	}
	if got := items.value("RO-META/STORY/ITEM").ExternalMetadata[0].Payload; got != `<value>I</value>` {
		t.Fatalf("item payload: %q", got)
	}
}
...
func TestMOS28RepeatedROCreateUpdatesCompositeItem(t *testing.T) {
	...
	for _, request := range []string{
		`<mos><mosID>openmos.example.test</mosID><ncsID>example.test</ncsID><messageID>80</messageID><roCreate><roID>RO-UPDATE</roID><roSlug>Update tracer</roSlug><story><storyID>STORY</storyID><storySlug>Story</storySlug><item><itemID>ITEM</itemID><itemSlug>Before</itemSlug><objID>OBJECT</objID><mosID>openmos.example.test</mosID><mosExternalMetadata><mosSchema>urn:example:item</mosSchema><mosPayload><value>before</value></mosPayload></mosExternalMetadata></item></story></roCreate></mos>`,
		`<mos><mosID>openmos.example.test</mosID><ncsID>example.test</ncsID><messageID>81</messageID><roCreate><roID>RO-UPDATE</roID><roSlug>Update tracer</roSlug><story><storyID>STORY</storyID><storySlug>Story</storySlug><item><itemID>ITEM</itemID><itemSlug>After</itemSlug><objID>OBJECT</objID><mosID>openmos.example.test</mosID><mosExternalMetadata><mosSchema>urn:example:item</mosSchema><mosPayload><value>after</value></mosPayload></mosExternalMetadata></item></story></roCreate></mos>`,
	} {
		writeMOS28ForTest(t, conn, request)
		var ack struct {
			ROAck struct {
	...
	if got := items.value("RO-UPDATE/STORY/ITEM").ExternalMetadata[0].Payload; got != `<value>after</value>` {
		t.Fatalf("updated item metadata = %q", got)
	}
}
...
<story><storyID>STORY-INVALID</storyID><storySlug>Must not persist</storySlug><item><itemSlug>Missing ID</itemSlug></item></story>
	...
	readMOS28XMLForTest(t, conn, &ack)
	if !strings.Contains(ack.ROAck.Status, "itemID is required") {
		t.Fatalf("ACK status = %q, want itemID validation reason", ack.ROAck.Status)
	}
...
<story><storyID>STORY</storyID><storySlug>Story</storySlug><item><itemID>ITEM</itemID><itemSlug>Item</itemSlug></item></story>
	...
	readMOS28XMLForTest(t, conn, &ack)
	if !strings.Contains(ack.ROAck.Status, "objID is required") {
		t.Fatalf("ACK status = %q, want objID validation reason", ack.ROAck.Status)
	}
